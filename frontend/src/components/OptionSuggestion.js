...
export default function OptionSuggestion({ signal, price, symbol, autoEnabled = false }) {
  ...
  const config = symbolConfig[symbol] || { name: symbol, lotSize: 1, prefix: '' };
  ...
  const fetchOpenTrade = useCallback(async () => {
    try {
      ...
      const active = data.trades?.find(t =>
  ...
  const handleBuy = async (optionName, buyPrice) => {
    ...
    try {
      await fetch(`${API}/trades`, {
        ...
        body: JSON.stringify({
          name: optionName, lot: selectedLots, quantity: totalQty,
          buy_price: buyPrice, sell_price: 0
        })
      });
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        ...
        if (data.error) { setError(data.error); } else { setOptions(data); setError(''); }
        setLoading(false);
      };
  ...
  const getFilteredOptions = () => {
    ...
    if (signal === 'NEUTRAL') {
      return { atm: options.options.find(o => o.label === 'atm_ce'), otm: options.options.find(o => o.label === 'atm_pe') };
    }
    const type = signal === 'BUY' ? 'CE' : 'PE';
    return {
      atm: options.options.find(o => o.label.startsWith('atm') && o.type === type),
      otm: options.options.find(o => o.label.startsWith('otm') && o.type === type)
    };
  };
    ...
    <div style={{ display: 'flex', gap: 12, alignItems: 'stretch' }}>
      ...
      <div style={tileStyle}>
        ...
        <div style={{
          ...
          marginBottom: 10, borderBottom: '1px solid #1e293b', paddingBottom: 8
        }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: '#64748b', letterSpacing: 1 }}>Signal</div>
            <div style={{ fontSize: 20, fontWeight: 'bold', color: signalColor[signal] || '#eab308' }}>{signal}</div>
          </div>
          ...
          <div style={{ flex: 1, textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700 }}>₹{Math.round(price)}</div>
          </div>
        </div>

        {/* Options rows */}
        {loading ? (
          <div style={{ textAlign: 'center', color: '#64748b', padding: 20 }}>Loading live options...</div>
        ) : error ? (
          ...
          <>
            ...
            {signal !== 'NEUTRAL' && best ? (
              <div style={{ textAlign: 'center', marginTop: 8 }}>
                <div style={{ fontSize: 9, color: '#94a3b8', letterSpacing: 1, marginBottom: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <span style={{
                    width: 6, height: 6, borderRadius: '50%',
                    background: connected ? '#22c55e' : '#ef4444',
                    display: 'inline-block', animation: connected ? 'pulse 1.5s infinite' : 'none'
                  }} />
                  RECOMMENDED
                </div>
                <div style={{ fontSize: 13, fontWeight: 800, color: signalColor[signal] }}>{config.prefix} {best.strike} {best.type}</div>
              </div>
            ) : (
              <>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 12, fontWeight: 800, color: '#eab308', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    <span style={{
                      width: 6, height: 6, borderRadius: '50%',
                      background: connected ? '#22c55e' : '#ef4444',
                      display: 'inline-block', animation: connected ? 'pulse 1.5s infinite' : 'none'
      ...
      <div style={tileStyle}>
          ...
          <>
            ...
            {!hasPosition && (
              <div style={{ textAlign: 'center', marginBottom: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 800 }}>{config.prefix} {best.strike} {best.type}</div>
                <div style={{ fontSize: 18, fontWeight: 800, color: signalColor[signal], marginTop: 4 }}>
                  ₹{best.ltp?.toFixed(2) || '—'}
                </div>
              </div>
            )}
            ...
            {hasPosition && (
              <div style={{
                ...
                <div style={{ fontSize: 9, color: '#f59e0b', letterSpacing: 0.5 }}>OPEN POSITION</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div>
                  ...
                  <select
                    ...
                    style={{
                      background: '#0f172a', color: hasPosition ? '#475569' : '#e2e8f0',
                      border: '1px solid #334155', borderRadius: 4,
                      padding: '4px 6px', fontSize: 14, fontWeight: 700,
                      cursor: hasPosition ? 'not-allowed' : 'pointer', outline: 'none', width: 50
                    }}
                  >
                ...
                <div>
                  <div style={{ fontSize: 9, color: '#64748b', letterSpacing: 0.5 }}>QTY</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#e2e8f0' }}>
                    {hasPosition ? (openTrade.quantity || openTrade.lot) : totalQty}
                  </div>
                </div>
              </div>
        ...
        /* ── NEUTRAL + open trade: sell only ── */
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: 6 }}>
              <div style={{ fontSize: 9, color: '#f59e0b', letterSpacing: 1 }}>OPEN POSITION</div>
        ...
        /* ── NEUTRAL + no trade + had prev signal: disabled ── */
          ...
          <div style={{ textAlign: 'center', opacity: 0.4, filter: 'grayscale(0.5)' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: '#475569', marginTop: 4 }}>
...
function OptionRow({ label, strike, type, ltp, color }) {
  return (
    <div style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      padding: '5px 8px', background: '#0f172a', borderRadius: 5, border: `1px solid ${color}`
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{
        ...
        <span style={{ fontSize: 12, fontWeight: 700 }}>{strike} {type}</span>
      </div>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b' }}>₹{ltp?.toFixed(2) || '—'}</div>
    </div>
  );
}
