...
export default function Settings() {
  ...
  // ── Trading settings (auto-start) ──────────────────────────
  const [autoStartPaper, setAutoStartPaper] = useState(false);
  const [tradingSettingMsg, setTradingSettingMsg] = useState('');
  const [tradingSettingLoading, setTradingSettingLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchTradingSettings = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setAutoStartPaper(!!data.auto_start_paper);
        }
      } catch (_) {}
    };
  ...
  const saveTradingSettings = async () => {
    setTradingSettingLoading(true);
    setTradingSettingMsg('');
    try {
      const res = await fetch('/user/settings', {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ auto_start_paper: autoStartPaper })
      });
      const data = await res.json();
      if (data.status === 'ok') {
        setTradingSettingMsg('Saved!');
      } else {
        setTradingSettingMsg(data.error || 'Error saving settings');
      }
    } catch (err) {
      setTradingSettingMsg(err.message);
    } finally {
      setTradingSettingLoading(false);
      setTimeout(() => setTradingSettingMsg(''), 3000);
    }
  };
      ...
      <div style={contentStyle}>
        {activeTab === 'account' && (
          <Section title="Account">
            <SettingItem>
              ...
              />
              <small
                style={{ color: '#475569', marginTop: 4, display: 'block' }}
              >
            ...
            {profileMsg && (
              <div
                style={{
                  ...
                  marginBottom: 16,
                  fontSize: 13,
                  background: profileMsg === 'Saved!' ? '#065f46' : '#7f1d1d',
                  color: profileMsg === 'Saved!' ? '#86efac' : '#fca5a5'
                }}
              >
                {profileMsg === 'Saved!'
        ...
        {activeTab === 'trading' && (
          <Section title="Trading Preferences">
            <SettingItem>
              ...
              >
                <div>
                  ...
                  >
                    Auto-start Paper Trading at 9:31 AM
                  </div>
                  <div
                    style={{ fontSize: 13, color: '#64748b', lineHeight: 1.5 }}
                  >
                    Server will automatically start paper trading for you every
                    morning at 9:31 AM IST.
                    <br />
                    You don&apos;t need to be logged in — it runs on the server.
                    <br />
                    <span style={{ color: '#f59e0b' }}>
                      Paper mode only. Real trading always requires manual
                      start.
                    </span>
                  </div>
                </div>
                <div
                  onClick={() => setAutoStartPaper((v) => !v)}
                  style={{
                    width: 48,
                    height: 26,
                    borderRadius: 13,
                    ...
                    flexShrink: 0,
                    marginLeft: 24,
                    background: autoStartPaper ? '#22c55e' : '#334155',
                    position: 'relative',
                    transition: '0.2s'
                  }}
                >
            ...
            {tradingSettingMsg && (
              <div
                style={{
                  ...
                  fontSize: 13,
                  background:
                    tradingSettingMsg === 'Saved!' ? '#065f46' : '#7f1d1d',
                  color: tradingSettingMsg === 'Saved!' ? '#86efac' : '#fca5a5'
                }}
              >
                {tradingSettingMsg === 'Saved!'
                  ? 'Settings saved!'
                  : tradingSettingMsg}
              </div>
            )}
            <button
              ...
              disabled={tradingSettingLoading}
              style={{
                padding: '10px 24px',
                borderRadius: 8,
                border: 'none',
                background: '#22c55e',
                color: 'black',
                cursor: tradingSettingLoading ? 'not-allowed' : 'pointer',
                fontWeight: 600,
                display: 'flex',
                alignItems: 'center',
                gap: 8,
                opacity: tradingSettingLoading ? 0.6 : 1
              }}
            >
              <Save size={16} />
              {tradingSettingLoading ? 'Saving...' : 'Save Trading Settings'}
            </button>
          </Section>
        )}
        ...
        {activeTab === 'api' && (
          <Section title="Kite API Credentials">
            <InfoBox>
              Save your Zerodha Kite API credentials securely. Your credentials
              are encrypted and used for trading orders only.
            </InfoBox>
            <div
              style={{
                padding: 16,
            ...
            >
              ...
              <small style={{ display: 'block', marginTop: 4 }}>
                {kiteCredentials.hasCredentials
                  ? 'Your Kite credentials are securely stored.'
            ...
            {credentialsMessage && (
              <div
                style={{
                  ...
                  fontSize: 13,
                  background:
                    credentialsMessage.includes('successfully') ||
                    credentialsMessage.includes('deleted')
                      ? '#065f46'
                      : '#7f1d1d',
                  color:
                    credentialsMessage.includes('successfully') ||
            ...
            >
              <SettingItem>
                ...
                >
                  Get this from Kite Console &rarr; Settings &rarr; API Consents
                </small>
              </SettingItem>
              <SettingItem>
            ...
            {!kiteCredentials.hasCredentials && (
              <InfoBox>
                When you provide credentials, all your trading orders will
                execute under your own Kite account in real-time.
              </InfoBox>
            )}
          </Section>
        )}
